...
def _timestamp():
    timeNow = time.time()
    timeStamp = datetime.datetime.fromtimestamp(timeNow).strftime('%Y-%m-%dT%H%M%S')
...
#    ax.axis('tight')
    ax.set_ylim(bottom = -0.1, top = ilen*0.5+0.5) #changed to 'top' from 'ymax' SM 2018-10-15
...
if __name__ == '__main__':
    ...
    fname1="GanttProject.csv"
    fname2="GanttCode.csv"

    blnXKCD = True  #TODO To be turned into args
    blnNow = True   #TODO To be turned into args
    _blnXCKD = blnXKCD
    _blnNow = blnNow
    
    #CreateGanttChart(fname)
    #CreateGanttChart(fname1)
    CreateGanttChart(fname2)


##SM Changes:
##TODO for this file:
    #Create Legend
    #XKCDify (as optional param)
    #Add vertical bars for today's date (another optional param) and milestones.
    #args (fname as string, now() as boolean, xkcd as boolean)

##Done for this file:
    #Swapped out ymin and ymax for bottom and top
